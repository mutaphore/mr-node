...
class Reader {
   ...
   * @param  {Object} config.sourceOptions - source-specific options: {path, host, port, user}
   */
  constructor(config) {
    ...
    switch (config.sourceType) {
      ...
      case 'hdfs':
        ...
        break;
      default:
        throw new Error("Unsupported resource type " + this.sourceType);
    }
  }
  /**
  ...
  createReadStream() {
    switch (this.sourceType) {
      ...
      case 'hdfs':
        ...
        return readline.createInterface({ 
          input: client.createReadStream(this.options.path) 
        });
      default:
        throw new Error("Unsupported resource type " + this.sourceType);
    }
